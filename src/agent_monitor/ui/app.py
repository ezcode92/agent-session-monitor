...
def _filters(snapshot):
    sessions=frame(snapshot["sessions"])
    ...
    with st.sidebar:
        ...
        agents=st.multiselect("에이전트",sorted(sessions.get("agent",pd.Series(dtype=str)).dropna().astype(str).unique()))
        ...
        refresh = st.toggle("전체 화면 5초 새로고침")
        st.caption(f"마지막 스캔: {snapshot['scanned_at']} · 표시 시간대: {timezone_name}")
...
def overview(snapshot,sessions,state,view=None):
    ...
    st.header("개요"); ids=set(sessions.get("session_id",pd.Series(dtype=str)).dropna()); usage=(view or {}).get("usage_df", filtered_usage(snapshot["usage"],state["start"],state["end"],state["agents"],state["projects"],state["models"],sessions)); requests=(view or {}).get("requests_df", filtered_requests(frame(snapshot["requests"]),state["start"],state["end"],state["agents"],sessions))
    ...
    st.dataframe(sessions,use_container_width=True,hide_index=True); st.download_button("세션 CSV",export_csv(sessions),"sessions.csv","text/csv")
...
def _live_monitor(agent: str, sid: str):
    ...
    events=frame(snapshot["events"]); full_rows=events[(events.session_id==sid) & (events.agent==agent)].to_dict("records") if {"session_id","agent"} <= set(events) else []
...
def history(snapshot,sessions,state,view=None):
    ...
    own=item.get("own_usage") or []; child=item.get("child_usage") or []; st.caption(f"직접 사용량 이벤트 {len(own)}건 · 하위 세션 사용량 이벤트 {len(child)}건")
    reqs=(view or {}).get("requests_df",frame(snapshot["requests"])); reqs=reqs[(reqs.session_id.astype(str)==sid) & (reqs.agent.astype(str)==agent)] if {"session_id","agent"} <= set(reqs) else reqs
    st.subheader("요청 사용량·시간·상태"); st.dataframe(reqs,use_container_width=True,hide_index=True); st.download_button("요청 CSV",export_csv(reqs),f"{choice}-requests.csv","text/csv")
    raw_events=(view or {}).get("events_df",frame(snapshot["events"])); raw_events=raw_events[(raw_events.session_id.astype(str)==sid) & (raw_events.agent.astype(str)==agent)] if {"session_id","agent"} <= set(raw_events) else raw_events
    monitor_id=f"{agent}:{sid}"; hide_noise=st.toggle("노이즈 이벤트 숨기기",value=True,key=f"history-noise:{monitor_id}")
    ...
    for event in normalized.tail(20).to_dict("records"):
        with st.expander(f"{event.get('occurred_at','')} · {event.get('source_label','')} · {event.get('record_key','')}"):
            st.text(lazy_preview(event))
            st.caption("원시 JSON은 자동으로 읽지 않습니다.")
            if st.button("원시 JSON 불러오기", key=f"raw:{choice}:{event.get('event_id') or event.get('record_key')}"):
                # The service opens the underlying JSONL record only for this
                # explicit drawer action; normal history/monitor rendering is lazy.
...
def analysis(snapshot,sessions,state,view=None):
    st.header("기간 분석"); ids=set(sessions.get("session_id",pd.Series(dtype=str)).dropna()); usage=(view or {}).get("usage_df",filtered_usage(snapshot["usage"],state["start"],state["end"],state["agents"],state["projects"],state["models"],sessions))
    ...
    comparison=(view or {}).get("comparison"); fallback_comparison, sentence=comparison_summary(usage,previous_usage)
    if comparison is None: comparison=fallback_comparison
...
def orchestration(snapshot, sessions, state, view=None):
    ...
    if roots:
        ...
        st.json(subtree[(subtree.agent==node[0]) & (subtree.session_id==node[1])].iloc[0].to_dict()); _live_monitor(node[0], node[1])
...
def run():
    ...
    try:
        ...
        if snapshot is None:
            snapshot=_snapshot(); st.session_state["dashboard_snapshot"]=snapshot
    except CoreContractError as error: st.error(f"코어 서비스 계약 오류: {error}"); st.stop()
    sessions,state=_filters(snapshot); view=build_view_data(snapshot,{**state,"start":state["start"],"end":state["end"]}); {"개요":overview,"작업 이력":history,"기간 분석":analysis,"오케스트레이션":orchestration,"설정":settings}[state["page"]](snapshot,sessions,state,view) if state["page"]!="설정" else settings(snapshot,sessions,state)
